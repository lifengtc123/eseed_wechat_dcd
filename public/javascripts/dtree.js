...
var iPath = "";
function Node(id, pid, name, url, title, target, color, open, pk, param, num, icon, iconOpen) {
	this.num = num;
	this.id = id;
	this.pid = pid;
	this.name = name;
...
dTree.prototype.init = function (iPath) {
	this.icon = {root:iPath + "/dtree/base.gif", folder:iPath + "/dtree/folder.gif", folderOpen:iPath + "/dtree/folderopen.gif", node:iPath + "/dtree/page.gif", empty:iPath + "/dtree/empty.gif", line:iPath + "/dtree/line.gif", join:iPath + "/dtree/join.gif", joinBottom:iPath + "/dtree/joinbottom.gif", plus:iPath + "/dtree/plus.gif", plusBottom:iPath + "/dtree/plusbottom.gif", minus:iPath + "/dtree/minus.gif", minusBottom:iPath + "/dtree/minusbottom.gif", nlPlus:iPath + "/dtree/nolines_plus.gif", nlMinus:iPath + "/dtree/nolines_minus.gif"};
	this.iPath = iPath;
};
dTree.prototype.add = function (id, pid, name, url, title, target, open, pk, param, num, icon, iconOpen) {
	if (typeof (iconOpen) != "undefined") {
		iconOpen = this.iPath + iconOpen;
	}
	if (typeof (icon) != "undefined") {
		icon = this.iPath + icon;
	}
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, target, open, pk, param, num, icon, iconOpen);
};
...
dTree.prototype.node = function (node, nodeId) {
	...
	if (node.url) {
		...
		str += "><font id=\"tr2\" name=\"" + node.title + "\" "+colorstr+" >";
	} else {
		if ((!this.config.folderLinks || !node.url) && node._hc && node.pid != this.root.id) {
			str += "<a href=\"javascript: " + this.obj + ".o(" + nodeId + ");\" class=\"node\"><font id=\"tr2\" name=\"" + node.id + "\"  "+colorstr+" >";
		}
	}
	str += node.name;
